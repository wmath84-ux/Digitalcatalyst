...
Remaining in the pack: **4 of 22 items not adopted** — `glass-switch`, `glass-slider`, `glass-popover` (Wave 4) and `glass-dock` (not adopted: §0 D4 keeps the repo's own dock for the footer nav and the desktop peek). Vendored verbatim so far: `glass` (2 files), `glass-button`, `glass-tabs`, `glass-dialog`, `glass-tooltip`, `glass-input`, `glass-card`, `glass-checkbox`, `glass-radio`, `glass-toggle-group`, `glass-accordion`, `glass-dropdown-menu`, `glass-select`, `glass-sheet`, `glass-tile`, `glass-swatch`, `glass-command`; plus `glass-toast` hand-ported (marked `PORTED` in the verify script). The fetch channel (`llms-full.txt`, plain text, one cache-busted request per chunk) is what made the Wave 3 transfer possible; it is still the only reliable one — the `/r/<item>.json` routes mangle JSX (§0 "Transfer reliability").

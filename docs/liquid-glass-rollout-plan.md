...
| `components/SearchBar.tsx` | sort control is `GlassSelect` (listbox, portaled, Escape + checkmark) | default export + `{value,onChange,sort,onSortChange}`, `data-store-search-trigger`, `role="button"`, Enter/Space, `#/search?q=`, clear, "Tap to search" | this was one of 13 native `<select>`s outside admin; 12 remain (flowpath 8, subscription 2, revision 1, `#/search` 1) and land in Waves 4–5 — admin's are out of scope by §0 |

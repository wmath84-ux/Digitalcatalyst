...
Remaining in the pack: **1 of 22 items not adopted** — `glass-dock` (not adopted: §0 D4 keeps the repo's own dock for the footer nav and the desktop peek). Vendored verbatim so far: `glass` (2 files), `glass-button`, `glass-tabs`, `glass-tooltip`, `glass-input`, `glass-dialog`, `glass-card`, `glass-checkbox`, `glass-radio`, `glass-toggle-group`, `glass-accordion`, `glass-dropdown-menu`, `glass-select`, `glass-sheet`, `glass-command`, `glass-tile`, `glass-swatch`, plus `glass-switch`, `glass-slider` and `glass-popover` (Wave 4); `glass-toast` stays hand-ported (marked `PORTED` in the verify script). The fetch channel (`llms-full.txt`, plain text, one cache-busted request per chunk) is what made the Wave 3 transfer possible; it is still the only reliable one — the `/r/<item>.json` routes mangle JSX (§0 "Transfer reliability").
...
**Wave 3 — commerce: DONE (store, product detail, cart, favourites, ⌘K palette).** 11 registry items landed in the same commit, so this wave is where the pack stops being "primitives in a drawer" and starts being what a shopper touches.

| Surface | Now renders | Commerce contract kept | Notes |
| --- | --- | --- | --- |
| `components/ProductCard.tsx` | `GlassCard` (`contentClassName="p-0"`) + `GlassSurface` wishlist disc + `LiquidMetalButton` CTA | `onView`, `onToggleWishlist`, `onAddToCart`, `stopPropagation`, all four CTA states and the `₹`/discount copy | the file's own fake "Glass sheen" gradient div is deleted — `GlassSurface` paints a real specular sheen, and keeping both doubles the backdrop-filter over a scrolling grid |
| `components/FilterChips.tsx` | `GlassToggleGroup` (`.dc-segment`) + `GlassSurface` panel + two `LiquidMetalButton`s | `filters/activeId/onSelect` (admin-driven chips), `title={description \|\| label}`, sideways scroll, `aria-expanded` | **the panel stays an in-place popover, not `glass-sheet`**: the sheet locks `body` scroll and this app's desktop shell owns its own scroller — a filter popover must not fight it |
| `components/SearchBar.tsx` | sort control is `GlassSelect` (listbox, portaled, Escape + checkmark) | default export + `{value,onChange,sort,onSortChange}`, `data-store-search-trigger`, `role="button"`, Enter/Space, `#/search?q=`, clear, "Tap to search" | this was one of 13 native `<select>`s outside admin; the other 12 landed in Wave 5 (flowpath 8, subscription 2, revision 1, `#/search` 1) — admin's stay out of scope by §0 |
| `PdpApp.tsx` `DetailsCard` | tab strip is the same `GlassToggleGroup` recipe | `data-pdp-tabbar`, the stuck shadow, `rounded-t-[23px]`, every `data-pdp-curriculum*` hook, `DetailTab` union | the curriculum module rows keep their own amber-vs-free identity: paid modules must not look like "selected glass" |
| `cartWishlist/CartItemCard.tsx` + `FavoriteCard.tsx` | `GlassCard` rows; favourites' Remove/Add pair is `LiquidMetalButton` | `onOpen`, `onRemove`, `onAddToCart`, `disabled={inCart}`, `formatINR` | "In Cart" is a *disabled action* now instead of a second colour scheme, so the state reads as unavailable rather than as a different button kind |
| ⌘K / Ctrl+K | `components/GlassCommandPalette.tsx` = `glass-command` (rail destinations from `ALL_RAIL`, four actions, ≤24 live catalogue titles, `GlassCommandEmpty`) | Wave 2's stand-in handler **deleted** from `DesktopShell` | the wrapper owns the keybinding (`shortcut={false}` on the pack item) because it must respect `data-glass="off"` and stay out of admin + the course player |

**Wave 6 — polish & hand-off: DONE. Rollout complete (Waves 0–6).** What this wave actually changed, and what it refused to:

| Item | Outcome |
| --- | --- |
| `#/home` hero actions | four hand-painted `border-white/35 bg-white/16 backdrop-blur-md` pills now carry `.dc-home-pill` (rim + top highlight + deep soft shadow) and the hero's last native `title="Profile"` became a glass tooltip; `ContinueLearning` rows take `.dc-card` |
| `#/checkout` | `PaymentGateway`: info + status cards take `.dc-card`, the amount card takes `.dc-quote` (rim only — a payment figure never sits on a translucent fill), the pay button keeps `bg-emerald-600` and gains an inset `GlassSurface` gloss with the label above it, the secondary action takes the repo's `dc-glass-soft` |
| FlowPath slider ink (correction) | Wave 4 shipped `dc-slider-on-dark` with `!important` on FlowPath's sliders. `src/flowpath/hooks/useTheme.ts` already writes `data-theme` on `<html>` (dark default, removed on unmount), so `useGlassDark()` picks the right palette by itself — and the forced rule was **wrong for FlowPath's light theme**. The class is gone there; it stays only on the course player, which has no theme attribute |
| a11y | `.dc-field/.dc-glass-select/.dc-segment/.dc-tile/.dc-home-pill` get a `:focus-visible` ring (keyboard-only); FlowPath's slider captions stopped being `<label>`s, because a `role="slider"` div is not labelable and the label pointed at nothing; the toast host was checked and **left alone** — each card is already `role="status"`/`"alert"`, so a wrapper `aria-live` would double-announce |
| reduced motion | one media block covers the pieces Waves 3–5 added (segment droplet, tile/switch press, pills, cards); the registry's own springs already honour `reduceMotion()`, and `detectGlassTier()` drops reduced-motion users to `lite` before anything mounts |
| lens budget | verified, not invented: no scrolling grid mounts a per-card `GlassLens` (store + home `ProductCard` are frost-only), so `claimLens`/`releaseLens` stay available-but-unused; a test pins the absence so a future per-card lens is a decision, not an accident |
| `GlassMaterial` cleanup | **declined.** §5 asked to delete the "now-dead" duplication: it is not dead — both `glass-dock` files import it, and `glass-dock` + `BottomNav` are frozen by decision D4 and the footer-nav constraint. The old bullet is now corrected in the table above and pinned by a test |
| hand-off | README gained a *Liquid Glass UI system* section (components, `.dc-*` doctrine, tiers, kill switch, sandbox, fidelity script, contracts); this doc is the wave record; PR opened from `arena/01a05c86-digitalcatalyst` |

Gate at this commit: `tsc` **7 errors** (= baseline, all in subscription/admin files this rollout never touched), `run_tests.sh` **1955 pass / 8 fail** (1963 total, +8 Wave-6 tests; the Wave 2 guard that pinned "home header is not touched yet" was updated to "touched by Wave 6, and only additively" — it is the one test in the repo that asserted a deliberate non-change, so it had to move with the decision), `npm run build` ok, frozen-path diff (`BottomNav`, `glass-dock`, `src/admin`, `src/components/admin`) **empty** — `src/home` and `src/checkout` intentionally leave the frozen list with this wave — dev server transforms every touched module with 0 errors. **21 of 22 pack items adopted; `glass-dock` remains the single, deliberate exception.**

---

**Wave 5 — player & account: DONE (last native controls, profile/settings rows, course confirm + seek).** The measurable outcome of this wave is arithmetic: **there is no native `<select>`, `type="range"` or `type="checkbox"` left anywhere in the app outside admin** — asserted by walking `src/` in `tests/liquidGlassWaveFiveContract.test.mjs`, so re-introducing one fails CI instead of shipping.

| Surface | Now renders | Contract kept | Notes |
| --- | --- | --- | --- |
| `flowpath/components/ActivityEditor.tsx` (7 selects) | one local `FieldSelect` helper on top of `glass-select` | every `setForm` union cast, all option values, `aria-label` per field | seven copies of one hand-styled recipe collapsed into one helper; the listbox is portaled, so it no longer gets clipped inside the modal's scroll container |
| `flowpath/components/BulkRevisionCreator.tsx` | `GlassSelect` per slot | difficulty still writes the preset's question count + minutes in the same handler | — |
| `revision/components/AiConfigForm.tsx` | model picker is `GlassSelect` | `disabled={allModels.length === 0}`, all three empty-state strings, the `(custom)` fallback row | the placeholder carries the copy the old fake single-`<option>` select used to show, so the "load models" gate still reads correctly |
| `components/SearchPage.tsx` | sort is `GlassSelect` | `SortKey` cast, `data-search-result-count` next to it | `#/search` and `#/store` now share one sort control instead of two dialects |
| `course/AudioPlayer.tsx` | seek bar is `GlassSlider` | `data-course-audio-seek/-current/-duration/-play/-mute/-loop` | `max={duration \|\| 1}` — the pack normalises by range, so a 0-duration metadata gap must not divide by zero |
| `course/ConfirmDeleteDialog.tsx` | the alert card **is** a `GlassSurface` (`tint 0.9`) | `role="alertdialog"`, `aria-modal`, backdrop-tap-cancels, `autoFocus`, all six `data-course-confirm-*` hooks | `bg-white shadow-2xl` became frost + rim; the destructive copy and top-anchored safe-area geometry are untouched |
| `profile/ProfileLayout.tsx` | `PreferenceRow` → `GlassSwitch`; `BaseModal` → `GlassSurface`; `INPUT` gains `.dc-field` | `{ checked, onChange }` API, `data-on`, sheet-vs-card corners, `required` / `inputMode` / `rows` / validation copy | one edit, two surfaces: `#/settings` imports both from profile. The switch's iOS-green default is re-inked to the app's indigo→violet in CSS, not by forking the component |
| `components/subscription/RenewalPreviewPage.tsx` | slider + 2 selects + checkbox | `data-preview-slider` and the other three probes `renewalPresentation` reads | developer sandbox, converted for parity so it previews what the live surfaces do |

One deliberate substitution, worth flagging in review: **profile fields do not wear `glass-input`.** That item is the pack's *search pill* — `radius = 9999`, a focus glow and no textarea twin — while the profile form needs `required`, `inputMode="numeric"`, `rows` and a 240-char counter. Forcing the anatomy would have cost more than it bought, so `.dc-field` in `src/glass.css` gives those fields the same frost + rim (`backdrop-filter: blur(10px) saturate(1.5)`, inset highlight, indigo focus ink) on the element that is already correct. `#/notifications`, `#/leaderboard`, `#/auth` and `#/landing` needed no wave-5 edit at all: they already render the Wave 1/2 primitives (`Modal`, `ConfirmDialog`, `Toast`, `PageTabs`, `LiquidMetalButton`), which is what "pack components on every page" should mean in practice.

Gate at this commit: `tsc` **7 errors** (= baseline), `run_tests.sh` **1947 pass / 8 fail** (1955 total, +8 Wave-5 tests; the same 8 pre-existing names), `npm run build` ok, frozen-path diff **empty**, dev server transforms all 13 touched modules with 0 errors.

---

**Wave 4 — learning surfaces: DONE (revision pickers, FlowPath sliders + toast, My Day tooltips).** This wave closes the last three gaps in the registry: `glass-switch`, `glass-slider`, `glass-popover` are vendored, so **21 of 22 items are now in the repo** (`glass-dock` stays deliberately unadopted per decision D4 — the mobile footer nav is frozen, and the desktop rail already uses the shell's own material).

| Surface | Now renders | Contract kept | Notes |
| --- | --- | --- | --- |
| `revision/pages/RevisionBankPage.tsx` | `GlassToggleGroup` (`.dc-segment`) for Saved Tests ↔ Smart Revision | `data-rev-bank-view-switch` on the wrapper, `setView`, `min-h-[42px]`, both icons | the hand-rolled pair painted two boxes that swapped backgrounds; one droplet now slides between them, matching the store's segments |
| `revision/pages/AiGeneratePage.tsx` | question-mode picker is a `GlassTile` grid | `data-rev-question-mode-grid`, `disabled={phase === "generating"}`, `min-h-[72px]`, emoji/label/desc stack | `aria-pressed` comes from the pack now — same semantics, one less bespoke state style |
| `revision/components/AiConfigForm.tsx` | provider picker cells are `GlassTile` | `aria-pressed` semantics, the ✓ badge, `meta.gradient` mark, **`meta.ring` per-provider accent** | the pack's selected accent is sky, invisible on white; `.dc-tile[data-selected]` re-inks it to indigo in `src/glass.css` |
| `flowpath/CurveSettingsModal.tsx` + `flowpath/CreateModal.tsx` | `GlassSlider` for every curve value and the progress field | same `min/max/step` scale, `onChange` → `onValueChange`, the `{progress}%` readout | FlowPath is a dark canvas without `.dark`, so `dc-slider-on-dark` re-inks rail/fill/thumb-shadow in CSS — the pack would otherwise paint a black rail on a black card |
| `flowpath/FlowPathView.tsx` | its local `{msg,key}` toast is gone; calls `toast.success` / `toast.info` | the "— coming soon" copy, the schedule confirmation text | `ToastViewport` is mounted once in `main.tsx` beside the palette host, so any route can raise a toast without prop plumbing |
| `components/myday/QuickNotes.tsx` | four native `title=` bubbles → `glass-tooltip` on the editor + note-row icon actions | `data-myday-note-editor-delete/-cancel`, `data-myday-note-save`, every colour and `onClick` | the cancel hint also lost a stale half-Hinglish string that had been shipped to users |

Two **deliberate non-changes**, both now pinned by `tests/liquidGlassWaveFourContract.test.mjs` so a later wave cannot "helpfully" undo them:

* **`revision/components/ui.tsx` stays as it is.** `tests/revisionProgressStableCardsContract.test.mjs` asserts `Card` renders `.rev-card rounded-3xl p-4` and *not* a blurred surface, and the CSS contract forbids `backdrop-filter` on `.rev-card` — a study app scrolls long cards all day, and the repo chose a stable painted surface for that. Glass landed on the controls students *touch*, not behind the content they read. `SecondaryButton` already carries the repo's own `dc-glass-soft`.
* **My Day's Create menu keeps its drop-up.** `tests/myDayCreateMenuDropdownContract.test.mjs` pins `bottom: calc(100% + 0.9rem)`, `role="menu"/menuitem`, the `w-56 -translate-x-1/2 sm:w-64` anchor, the staggered item animation and dismissal on Escape **plus** `pointerdown`/`scroll`/`touchmove`/`wheel`. `glass-popover` portals below the trigger, re-places on scroll instead of closing, and dismisses only on outside mousedown + Escape — swapping it would regress three documented behaviours for zero visual gain (the panel already uses the repo's frost class). It is vendored and live in the preview page for new surfaces.

`LOCAL_ADAPTATIONS` grew the three type-only entries the Wave 4 files needed (`React.PointerEvent` / `React.KeyboardEvent` / `React.RefObject` → imported aliases); no behaviour differs from the registry.

Gate at this commit: `tsc` **7 errors** (= baseline), `run_tests.sh` **1939 pass / 8 fail** (1947 total, +10 Wave-4 tests; the same 8 pre-existing names — one of which, `tests/flowpathFirstPaintContract.test.mjs`, fails at HEAD too because it reads `src/components/flowpath/Header.tsx`, a file the repo does not have), `npm run build` ok, frozen-path diff **empty**, dev server transforms all 12 touched modules with 0 errors.

---

Two things this wave changed **outside** the rollout's own files, both worth a look during review:

* `tests/storeFiltersAdminProductContract.test.mjs` ("store surfaces use frosted glass, colour and depth") asserted the literal utility classes `backdrop-blur` / `bg-white/\d` / `shadow-`. The frost is still there — it is just the pack's inline `backdrop-filter` + rim now, so the assertion accepts either implementation instead of one spelling. The intent of the test is unchanged and StorePage/Hero are still checked exactly as before.
* `src/glass.css` grew the `.dc-choice` / `.dc-tile` rules with `:not([aria-checked="true"])` / `:not([data-selected])` guards. Unlayered CSS beats the pack's utilities, so an unguarded rule would have flattened the accent-filled *checked* state — the guard is load-bearing and pinned by a Wave 3 test.

Gate at this commit: `tsc` **7 errors** (= baseline), `run_tests.sh` **1929 pass / 8 fail** (the same 8 baseline names; +13 new Wave-3 tests, 1937 total), `npm run build` ok, frozen-path diff (`BottomNav` ×4, `glass-dock`, `src/admin`, `src/components/admin`, `src/home`, `src/checkout`) **empty**, dev server transforms all 19 touched/vendored modules with 0 errors.
...
| UI surface size (non-admin) | **424** `<button>`, **71** `<input>`, **13** `<select>` *(0 left since Wave 5)*, **9** `<textarea>`, **16** `fixed inset-0` overlays in `components/`, **~600** `rounded-xl/2xl/3xl` class sites, **~120** `backdrop-blur` sites |
...
| **3. Commerce** ✅ | `#/store` (cards, filter droplet, sort listbox), `#/product/:id` (tab strip), `#/cart` + `#/favorites` (rows + actions), ⌘K palette (`glass-command`). `#/home`'s own header/cards and `#/checkout`'s gateway copy were still queued for Wave 5 but stay untouched — both files were green on every pinned contract and Wave 5's budget went to the 12 native controls; they move to Wave 6 with the polish pass; no `glass-checkbox` call-sites exist in cart/favourites to convert | 11 vendored, 8 edited | same + hover/keyboard pass |
| **4. Learning** ✅ | `#/revision` (segmented view switch, tile pickers, provider tiles), `#/flowpath` (both sliders, inline toast → shared host), `#/my-day` (note tooltips). Vendored `glass-switch` / `glass-slider` / `glass-popover`. **Deliberately not swapped:** revision's `.rev-card` surface (pinned non-blurred), My Day's Create menu drop-up (pinned geometry + dismissal) | 3 vendored, 8 edited | same + offline/empty states |
| **5. Player & account** ✅ | the last 12 native `<select>`s (FlowPath editor + bulk creator, model picker, search sort, renewal preview), the course player's seek bar + delete confirmation, profile/settings `PreferenceRow` → `glass-switch`, `BaseModal` → `GlassSurface`, `.dc-field` form ink. `#/notifications`, `#/leaderboard`, `#/auth`, `#/landing` needed nothing new — they render Wave 1/2 primitives already | 10 edited | same + portrait-lock regression check |
| **6. Polish & hand-off** ✅ | `#/home` hero pills + `ContinueLearning` → `.dc-home-pill` / `.dc-card`; checkout `PaymentGateway` keeps its emerald/indigo identity under a pack gloss layer; FlowPath's forced dark slider ink **removed** (its own `useTheme` already sets `data-theme`, and the rule broke FlowPath's light theme); a11y + reduced-motion + focus-visible guards; README + this doc; PR | 7 edited, 2 docs | full gate + PR |

...
- **The meadow's proven material recipe.** First revision shipped solid
  untextured cards and the owner reported them BLACK. The sward now wears
  exactly what the known-green meadow blade field wears — the engine's
  `grassBlade` texture as `map` with `alphaTest: 0.5` (deep-green root →
  sunlit-lime tip gradient, the "natural green" directive) — each clump is
  a crossed pair of alpha-tested blade cards, and the tint is the meadow's
  own hue-0.30 spring-summer green via `setColorAt`. Same recipe, same
  colour, guaranteed green.

...
const App: React.FC = () => {
      ...
      const unsubscribe = onAuthStateChanged(auth, user => {
          if (user) {
              ...
              void completeFirebaseUserSession(user, { redirect: false }).then(hydratedUser => {
                  ...
                  if (hydratedUser && currentViewRef.current === 'auth') redirectAfterSuccessfulAuth({ source: 'session-restore', user: hydratedUser, force: true });
              }).catch(error => {
                  setIsAuthStateReady(true);
  ...
  const renderPage = () => {
    // Mobile users can browse the home experience immediately; auth is opened only from explicit CTAs or protected actions.

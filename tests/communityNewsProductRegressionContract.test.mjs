...
test('desktop sidebar stays visible on wide desktop pages while Community uses the same desktop sidebar layout', () => {
...
test('Status hub keeps Create Story top-right and removes the Rules button from the hero', () => {
  ...
  assert.match(community, /mt-3 flex gap-2 overflow-x-auto pb-1 custom-scrollbar xl:hidden/);
...
test('story captions render below images and profile story text stays compact/readable', () => {
  ...
  assert.match(community, /community-status-story-text-preview/);
  assert.match(community, /font-size: clamp\(0\.62rem, 2\.4vw, 0\.78rem\)/);
...
test('Home News and Blog fallback cards keep only the real top overlay badge', () => {
  assert.match(latestNews, /absolute left-2 top-2/);
  assert.doesNotMatch(latestNews, /const badge = escapeSvgText\(isNews \? 'NEWS' : 'BLOG'\)/);
  assert.doesNotMatch(latestNews, /<rect x="78" y="74" width="220" height="58"/);
});

test('Admin side-panel visual settings are removed while HomeSideDock keeps hardcoded defaults', () => {
  assert.doesNotMatch(settings, /Website side panel font/);
  assert.doesNotMatch(settings, /Website side panel only/);
  assert.doesNotMatch(settings, /Side panel transparency/);

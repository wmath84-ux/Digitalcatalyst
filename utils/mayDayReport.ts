...
<title>My Day Performance Report — ${escapeHtml(displayName)}</title>
<style>
...
<div class="page">
  <header class="hero">
    <p class="kicker">My Day · Personal Performance Report</p>
  ...
  <section class="section">
    ...
    <p class="lead">Across all your My Day activity you completed <strong>${totalCompletedTasks} of ${totalTasks} tasks</strong> (${taskCompletionRate}%), logged <strong>${totalFocusMinutes} focused minutes</strong> across ${focusSessionCount} sessions, and kept <strong>${workspace.goals.length} goal${workspace.goals.length === 1 ? '' : 's'}</strong> in motion. This week you were active on ${activeDays} of 7 days${currentStreak ? ` with a ${currentStreak}-day streak` : ''}.</p>
    <div class="stats">
  ...
  <footer>Generated by My Day · Eduvora. Small steps today create the progress you will be proud of tomorrow.</footer>
</div>
</body>
</html>`;
};

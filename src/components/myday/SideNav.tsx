 ...
 *
 * WIDTH — the panel is a 244px column only from 1280px up. From 768 to
 * 1279px it is a compact ICON RAIL (~68px): a small tablet has the room for
 * one narrow column of controls, not for a full panel, and the old 224px
 * panel left 448px for the page — narrower than a phone — so every My Day
 * page visibly shrank the moment the rail appeared (and again at 1024px).
 * On the rail every section is still present and clickable, with its label
 * on the button's `title`/`aria-label`; the labels, the month calendar and
 * the quote of the day come back with the full panel at 1280px, where the
 * page column is wide enough to give them their space.
 */
export default function SideNav({ active, onNavigate }: SideNavProps) {
  const quote = quoteOfTheDay();
  return (
    <aside className="sticky top-[65px] hidden h-fit w-[4.25rem] shrink-0 md:block xl:w-60">
      <GlassSurface
        radius={24}
        className="dc-scene-plate text-white"
        contentClassName="flex max-h-[calc(100dvh-100px)] flex-col gap-1 overflow-y-auto custom-scrollbar p-2 xl:p-4"
      >
        {/* Panel identity — full panel only (the rail has no room for it). */}
        <div className="mb-3 hidden px-1 pt-0.5 xl:block">
        ...
        <nav className="myday-snav flex flex-col gap-1.5" aria-label="My Day sections">
          {items.map((item) => {
            ...
            return (
              <button
                ...
                // The rail hides the visible label, so the accessible name
                // and the hover tooltip carry it there.
              ...
              >
                ...
                <span className="myday-snav-label min-w-0 flex-1 truncate">{item.label}</span>
                {isActive && (
        ...
        <MyDayCalendar className="mt-4 hidden xl:block" />

        <figure className="myday-quote-card mt-4 hidden xl:block">

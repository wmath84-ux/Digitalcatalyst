...
export default function MyDayAllowanceCard({ onOpenMyDay, onSubscribe }: Props) {
  ...
  return (
    <section
      ...
      aria-live="polite"
      className="relative rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_14px_40px_-24px_rgba(49,46,129,0.35)] lg:rounded-2xl lg:p-3.5 lg:shadow-sm"
    >
      <div className="relative">
        <div className="flex items-start justify-between gap-3">
          <div className="flex min-w-0 items-center gap-3">
            <div className="grid h-12 w-12 shrink-0 place-items-center rounded-2xl bg-gradient-to-br from-indigo-600 to-violet-600 text-white shadow-md shadow-indigo-500/30 lg:h-9 lg:w-9 lg:rounded-xl">
          ...
          <div className="flex shrink-0 items-center gap-1.5">
            <button
              ...
              className="grid h-8 w-8 place-items-center rounded-full border border-slate-200 bg-white text-indigo-600 shadow-sm transition hover:bg-indigo-50 disabled:opacity-50"
            >
            ...
            <div className="mt-5">
              ...
              <div className="mt-2 h-2.5 overflow-hidden rounded-full bg-slate-100 ring-1 ring-slate-200/70">
                <div
                  ...
                  className={`h-full rounded-full transition-all duration-500 ${browseOnlyPlan ? "bg-slate-300" : exhausted ? "bg-amber-500" : "bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600"}`}
            ...
            {browseOnlyPlan ? null : (
              <>
                <div className="mt-4 grid grid-cols-3 gap-2">
                  <Stat label="Used" value={`${freeUsed}`} tone="bg-slate-50 ring-slate-100" />
                  <Stat label="Left" value={`${freeRemaining}`} tone="bg-slate-50 ring-slate-100" />
        ...
        <div className="mt-4 grid gap-2">
          <button
            ...
            onClick={onOpenMyDay}
            className="flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-indigo-600 via-violet-600 to-fuchsia-600 py-3.5 text-sm font-black text-white shadow-lg shadow-indigo-500/30 transition hover:brightness-110 active:scale-[0.99]"
          >

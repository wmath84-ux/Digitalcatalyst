...
function HiddenFeatureHintImpl({ featureName, onViewSubscription, className }: Props) {
  ...
  return (
    <GlassSurface
      ...
      role="status"
      className={"mx-auto w-full max-w-md text-white " + (className || "")}
      contentClassName="flex flex-col items-center gap-3 px-6 py-6 text-center"
    >
      <span
        aria-hidden
        className="inline-flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-600 text-white"
      >
      ...
      <button
        type="button"
        onClick={handleClick}
        className="inline-flex w-full items-center justify-center rounded-full bg-indigo-600 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-indigo-500 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400"
      >

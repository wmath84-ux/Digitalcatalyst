    ...
    <div className="min-h-screen bg-slate-100 text-slate-900 sm:py-6">
      <div className="relative mx-auto flex min-h-screen w-full max-w-md flex-col bg-white shadow-xl shadow-slate-200 sm:min-h-[calc(100vh-3rem)] sm:overflow-hidden sm:rounded-[2rem] sm:border sm:border-slate-200">
        ...
        <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="flex items-center justify-between"><div className="grid h-11 w-11 place-items-center rounded-2xl bg-violet-100"><Crown className="text-violet-600" /></div><span className="rounded-full bg-emerald-100 px-2 py-1 text-[10px] font-black uppercase text-emerald-700">{user.subscriptionTier || "basic"}</span></div><p className="mt-4 text-xs font-bold text-slate-400">Membership</p><p className="text-xl font-black capitalize">{user.subscriptionTier === "basic" ? "Basic learner" : `${user.subscriptionTier} membership`}</p><button onClick={() => { window.location.hash = "#/subscription"; }} className="mt-4 flex items-center gap-1 text-xs font-black text-violet-700">Manage membership <ChevronRight size={14} /></button>{referralCode && (referralUsed
          ? <div data-profile-referral-used className="mt-4 rounded-2xl bg-slate-100 p-3"><div className="flex items-center justify-between"><p className="text-[10px] font-black uppercase tracking-wider text-slate-400">Your referral code</p><span className="rounded-full bg-amber-200 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-amber-800">Used</span></div><div className="mt-1 flex items-center justify-between gap-2"><code className="min-w-0 truncate text-sm font-black text-slate-400 line-through decoration-2 decoration-rose-400">{referralCode}</code><X size={14} className="shrink-0 text-rose-500" aria-label="Referral code used" /></div><p className="mt-1 text-[10px] font-semibold text-slate-400">This referral ID has been used and is no longer active.</p></div>
          : <div className="mt-4 rounded-2xl bg-violet-50 p-3"><p className="text-[10px] font-black uppercase tracking-wider text-violet-500">Your referral code</p><div className="mt-1 flex items-center justify-between gap-2"><code className="min-w-0 truncate text-sm font-black text-violet-900">{referralCode}</code><button onClick={() => void navigator.clipboard?.writeText(referralCode)} className="rounded-lg bg-white px-2 py-1 text-[10px] font-bold text-violet-700">Copy</button></div></div>)}</section>

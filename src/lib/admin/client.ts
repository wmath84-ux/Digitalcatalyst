...
const resourceToCourse = (resource: ProductResource) => ({
  id: resource.id, name: resource.name,
  type: ({ video_url: "video", audio_url: "audio", image_url: "image", gdrive: "embed", gdoc: "doc", gsheet: "sheet", gform: "google_form", github_pages: "embed", whimsical: "mindmap", iframe: "embed" } as Record<string, string>)[resource.type] || resource.type,
  url: resource.url, embedUrl: resource.url, provider: resource.provider, accessLevel: resource.accessLevel === "paid_update" ? "paidUpdate" : resource.accessLevel === "hidden" ? "hidden" : "included",
  paidUpdateId: resource.paidUpdateId || undefined, paidUpdatePrice: resource.cashPrice == null ? undefined : `₹${resource.cashPrice}`, paidUpdateCoinPrice: resource.coinPrice || 0,
});
const moduleToCourse = (module: ProductModule, all: ProductModule[]): any => ({
  id: module.id, title: module.title, description: module.description, files: module.resources.map(resourceToCourse),
  modules: all.filter((item) => item.parentModuleId === module.id).sort((a,b)=>a.sortOrder-b.sortOrder).map((item) => moduleToCourse(item, all)),
  accessLevel: module.accessLevel === "paid_update" ? "paidUpdate" : module.accessLevel === "hidden" ? "hidden" : "included",
  paidUpdateId: module.entitlementId || undefined, paidUpdatePrice: module.cashPrice == null ? undefined : `₹${module.cashPrice}`, paidUpdateCoinPrice: module.coinPrice || 0,
  individuallyPurchasable: module.individuallyPurchasable, cashPrice: module.cashPrice, salePrice: module.salePrice, coinPrice: module.coinPrice,
  includeInBundle: module.includeInBundle, previewAvailable: module.previewAvailable, requiredPreviousModuleIds: module.requiredPreviousModuleIds,
});

function productForEditor(raw: any, documentId: string) {
  if (raw.adminProduct) return { ...raw.adminProduct, id: documentId };
  const images = (raw.images || []).map((url: string, index: number) => ({ id: `img-${index}`, url, provider: url.includes("cloudinary") ? "cloudinary" : "public", sortOrder: index, isPrimary: index === 0 }));
  return { id: documentId, title: raw.title || "", shortDescription: raw.description || "", longDescription: raw.longDescription || "", instructor: raw.instructor || "Digital Catalyst", category: raw.category || "", productType: "course", classLevel: raw.dimensions || "", subject: raw.subject || "", sku: raw.sku || "", tags: raw.tags || [], searchKeywords: raw.keywords || [], features: raw.features || [], estimatedDuration: raw.dimensions || "", language: "", manualRating: raw.manualRating, visibility: raw.isVisible === false ? "hidden" : "visible", availableForSale: raw.inStock !== false, images, regularPrice: String(money(raw.price)), salePrice: raw.salePrice ? String(money(raw.salePrice)) : null, isFree: Boolean(raw.isFree), coinPurchaseEnabled: Boolean(raw.coinPrice), coinPrice: Number(raw.coinPrice || 0), eligibleCouponIds: raw.couponCode ? [raw.couponCode] : [], minPayableAmount: "0", availabilityDate: null, saleStart: null, saleEnd: null, modules: [], paidUpdates: [], status: raw.isVisible === false ? "draft" : "published" };
}
...
async function productsRequest(url: URL, init?: RequestInit) {
  ...
  let products = snap.docs.map((item) => { const p = productForEditor(item.data(), item.id); return { ...p, reviewCount: Number(item.data().reviewCount || 0), rating: String(item.data().rating || item.data().manualRating || 0), updatedAt: asDate(item.data().updatedAt), modules: p.modules, images: p.images }; });
...
async function saveProduct(ref: ReturnType<typeof doc>, body: any) {
  const modules: ProductModule[] = body.modules || [];
  const roots=modules.filter((m)=>!m.parentModuleId).sort((a,b)=>a.sortOrder-b.sortOrder).map((m)=>moduleToCourse(m,modules));
  const urls=(body.images||[]).sort((a:any,b:any)=>a.sortOrder-b.sortOrder).map((i:any)=>i.url);
  await setDoc(ref,{ adminProduct: body, id: ref.id, title: body.title, description: body.shortDescription, longDescription: body.longDescription, instructor: body.instructor, category: body.category, subject: body.subject, sku: body.sku, tags: body.tags||[], keywords: body.searchKeywords||[], features: body.features||[], images: urls, productImages: {card: urls.find((_:any,i:number)=>body.images[i]?.isPrimary)||urls[0]||""}, price:`₹${body.regularPrice||0}`, salePrice: body.salePrice?`₹${body.salePrice}`:null, coinPrice: body.coinPrice||0, isFree:Boolean(body.isFree), isVisible:body.visibility==="visible", inStock:Boolean(body.availableForSale), manualRating:body.manualRating, dimensions:body.estimatedDuration, courseContent:roots, paidUpdates:body.paidUpdates||[], updatedAt:serverTimestamp() },{merge:true});
}
...
async function customersRequest(url: URL, init?: RequestInit) {
  ...
  let rows=(await getDocs(collection(db,"users"))).docs.map(d=>mapCustomer(d.id,d.data())); const q=(url.searchParams.get("q")||"").toLowerCase();if(q)rows=rows.filter((r:any)=>`${r.uid} ${r.name} ${r.email}`.toLowerCase().includes(q));const s=url.searchParams.get("status");if(s)rows=rows.filter((r:any)=>r.status===s);const p=url.searchParams.get("provider");if(p)rows=rows.filter((r:any)=>r.provider===p);return {customers:rows};
}

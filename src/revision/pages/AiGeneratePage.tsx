...
export default function AiGeneratePage({ uid, route }: Props) {
  ...
  const runGenerate = async () => {
    ...
    try {
      ...
      const title =
        subjectNames.length === 1 ? `AI Test · ${subjectNames[0]}` : `AI Test · ${subjectNames.length} subjects`;
  ...
  return (
    <PageShell route={route} title="AI Test Generator" subtitle="Build your own exam in one tap" backHref="#/revision/profile">
            ...
            {phase === "generating" ? (
              ...
              <>
                <PrimaryButton disabled={!canGenerate} onClick={() => void runGenerate()}>
                  <SparklesIcon className="h-5 w-5" /> Generate my exam
                </PrimaryButton>
                {!canGenerate && (
        ...
        {phase === "ready" && readyInfo && (
          <Card className="overflow-hidden border-emerald-200 bg-gradient-to-br from-emerald-50 via-white to-teal-50">
            <div className="flex flex-col items-center gap-3 py-4 text-center">
              ...
              <div>
                <h2 className="text-lg font-bold text-slate-900">Exam is ready right now! 🎉</h2>

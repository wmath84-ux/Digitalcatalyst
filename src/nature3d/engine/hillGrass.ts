...
// `grass.ts`):
//
//   * ONE 4-triangle bent card per clump, shared by every instance. The CPU
//     never touches a clump after boot.
...
/**
 * One bent, tapered clump card: a 1 m × 1 m plane with two height segments
 * (so the wind can BEND it, not just tilt it), rooted at the origin and
 * curled slightly forward like a real blade. Unit sized: the instance scale
 * carries the metres.
 */
...
export function createHillGrassField(
  ...
  const plant = (x: number, z: number, y: number, t: number): void => {
    ...
    // Colour: the ground's own rule, then pulled firmly back to grass —
    // the hills must read as the SAME meadow climbing, not as a new biome.
    // High ground gets a touch more light and a hint less saturation: the
    // sun-bleached alpine sward, still green from base to crest.
    groundColorAt(x, z, y, ground, GROUND_PALETTE, 1, 0);
    ground.getHSL(hsl);
    const patch = (Math.sin(x * 0.13) * Math.cos(z * 0.11) + 1) * 0.5;
    const altitude = Math.min(1, Math.max(0, y / 100));
    const hue = 0.3 + hsl.l * 0.018 + patch * 0.012 + (Math.random() - 0.5) * 0.028 - altitude * 0.012;
    const sat = Math.min(0.92, 0.68 + hsl.s * 0.2 + patch * 0.06 + Math.random() * 0.07 - altitude * 0.1);
    const lit = 0.46 + hsl.l * 0.24 + Math.random() * 0.1 + altitude * 0.08;
    color.setHSL(hue, sat, Math.min(0.78, lit));

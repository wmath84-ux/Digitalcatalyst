...
export interface MossBank {
  ...
  /**
   * Thermal fail-safe: level ≥ 1 hides the bank entirely (it reappears if
   * the ladder ever relaxes — visibility only, zero churn).
   */
...
/**
 * Anchor spacing along the bank, per tier. The low tier relaxes "one clump
 * per metre" to one per ~2.4 m: twelve variants of alpha-tested clumps every
 * metre on BOTH banks was one of the quiet overdraw factories on phones.
 */
  ...
  return new Promise<MossBank>((resolve, reject) => {
    loader.load(
      ...
      (gltf) => {
        ...
        //
        // LOW TIER (`cheapPlants`): Lambert + diffuse + alpha only — no
        // normal/ARM/AO fetches, no PBR ALU (same diet as sorrel + tufts).
        ...
        material.customProgramCacheKey = () => (budget.cheapPlants ? "dc-moss-diet" : "dc-moss");
        ...
        const meshes = variants.map((g) => {
          ...
          mesh.castShadow = false; // foliage rule: never in the shadow pass

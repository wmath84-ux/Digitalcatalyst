...
export interface GrassField {
  ...
  /**
   * Thermal fail-safe detail ladder (see scene.ts). Trimming an
   * InstancedMesh's `count` is FREE — the buffers stay allocated, the GPU
   * simply draws fewer instances — so the far ring can shrink the moment the
   * device runs hot: no re-scatter, no hitch, no pop-in risk on recovery.

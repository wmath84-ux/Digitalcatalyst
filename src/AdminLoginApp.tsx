...
export default function AdminLoginApp() {
  const { user, loginAdmin, resetPassword } = useAuth();
  ...
  const sendReset = async () => {
    setError("");
    setInfo("");
    const target = email.trim() || "wmath84@gmail.com";
    setResetting(true);
    try {
  ...
  return <main className="grid min-h-[100dvh] place-items-center bg-[#05060f] px-4 py-8 text-white">
    ...
    <section className="w-full max-w-md rounded-3xl border border-white/10 bg-white/5 p-6 shadow-2xl backdrop-blur-xl sm:p-8">
      ...
      <p className="mt-2 text-sm leading-6 text-slate-400">Login needs three things: approved email <span className="text-slate-200">wmath84@gmail.com</span>, the Firebase Authentication password for that user, and <span className="text-slate-200">users/&#123;uid&#125;.role = admin</span> in Firestore.</p>
      ...
      <p className="mt-5 text-center text-[10px] leading-5 text-slate-500">Use #/admin-login. The password is the one in Firebase Authentication, not a Firestore field. Dashboard access is not remembered after this session closes.</p>
    </section>
  </main>;
}

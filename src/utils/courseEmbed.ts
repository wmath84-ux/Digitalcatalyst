...
// ---------------------------------------------------------------------------
// Email-gate resource resolution
// ---------------------------------------------------------------------------

/** Google resource families that can be fulfilled by the owner-side gate. */
export type GateResourceKind = "doc" | "sheet" | "slides" | "drive";

/** Return the resource family used by the email-only personal-access gate. */
export const gateResourceKind = (file: CourseFile): GateResourceKind | null => {
  const google = googleParts(getCourseFileUrl(file));
  if (google?.kind === "document") return "doc";
  if (google?.kind === "spreadsheets") return "sheet";
  if (google?.kind === "presentation") return "slides";
...
/** Extract the source ID sent to the owner-controlled gate workflow. */
export const getGateSourceFileId = (file: CourseFile): string => {
  const google = googleParts(getCourseFileUrl(file));
  if (!google || google.kind === "forms") return "";
  return google.id;
};

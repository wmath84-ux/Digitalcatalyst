...
export default function CartPage({ cartProducts, onRemove, onClearAll, onCheckout, onNavigate, onRequireAuth, onOpenProduct }: CartPageProps) {
  ...
  return (
    <div className="flex h-full flex-col">
      <PageHeader title="My Cart" subtitle={`${cartProducts.length} item${cartProducts.length > 1 ? "s" : ""}`} right={<button onClick={onClearAll} className="flex items-center gap-1 rounded-full border border-white/70 bg-rose-50/80 px-3 py-1.5 text-[11px] font-bold text-rose-500 backdrop-blur"><Trash2 size={12} /> Clear</button>} />
      <div data-cart-row className="flex-1 overflow-y-auto px-4 pb-4 pt-3">
        ...
        <div className="dc-glass mt-4 rounded-[1.6rem] p-4 shadow-[0_18px_42px_-24px_rgba(79,70,229,0.42)]">
          ...
          <div className="space-y-2 text-[13px]">
            ...
            <div className="border-t border-dashed border-white/10 pt-3 flex justify-between"><span className="font-extrabold">Total payable</span><span className="text-lg font-extrabold text-indigo-600">{formatINR(finalTotal)}</span></div>
          </div>
...
function PageHeader({ title, subtitle, right }: { title: string; subtitle: string; right?: React.ReactNode }) {
  return <div className="dc-glass-toolbar flex items-center justify-between border-b border-white/60 px-4 py-4"><div><h1 className="text-lg font-extrabold text-white">{title}</h1><p className="text-[11px] font-medium text-white/55">{subtitle}</p></div>{right}</div>;
}

...
const BottomGlassDock: React.FC<BottomGlassDockProps> = ({ settings, currentUser, purchasedProducts, cartCount, wishlistCount, onOpenBlogModal, onOpenFreeModal, onOpenAnnouncementsModal, onNavigateToAllProducts, onNavigateToWishlist, onNavigateToPurchases, onCartClick, onProfileClick, onSubscriptionClick }) => {
  ...
  return (
    <div className="fixed inset-x-0 bottom-2 md:bottom-4 z-[65] flex justify-center pointer-events-none px-3">
      <div className="pointer-events-auto group relative max-w-[95vw] overflow-hidden rounded-[2rem] border border-white/15 bg-slate-950/80 px-3 py-3 shadow-[0_24px_80px_rgba(15,23,42,0.34)] ring-1 ring-slate-900/10 backdrop-blur-3xl transition-all duration-500 hover:-translate-y-0.5 hover:bg-slate-950/85 hover:shadow-[0_28px_90px_rgba(30,41,59,0.38)] data-[hidden=true]:translate-y-24 md:px-4" id="main-bottom-dock">
        ...
        <div className="relative flex items-center gap-2 overflow-x-auto pb-1 custom-scrollbar">
          {items.map((item) => {
            ...
            return (
              <button key={item.label} onClick={item.action} className={`group/item relative flex min-w-[86px] flex-col items-center rounded-2xl border border-white/10 bg-white/[0.08] px-3 py-2.5 text-slate-100 shadow-[inset_0_1px_0_rgba(255,255,255,0.10)] transition duration-300 hover:-translate-y-1 hover:bg-white/[0.14] hover:shadow-[0_14px_32px_rgba(15,23,42,0.28)] md:min-w-[92px] ${tone}`}>
                <span className={`absolute inset-0 rounded-2xl bg-gradient-to-br ${tone.split(' hover:')[0]} opacity-45 transition duration-300 group-hover/item:opacity-75`} />

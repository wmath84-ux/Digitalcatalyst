...
const WishlistPage: React.FC<WishlistPageProps> = ({ settings, products, onViewProduct, wishlist, onToggleWishlist, onNavigateToAllProducts, onAddToCart, onBuyNow, onClearWishlist, coupons, purchasedProductIds = [] }) => {
  ...
  if (!settings.features.showFavourites) {
    return (
        <div className="py-20 sm:py-24 bg-white/70 backdrop-blur-xl min-h-[60vh] flex items-center justify-center">
        ...
        <div ref={gridRef} className={`mt-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12 ${settings.animations.enabled ? 'stagger-animate-container' : ''}`}>
          {products.length > 0 ? (
            ...
            <div className="col-span-full text-center py-12">
              ...
              <button onClick={onNavigateToAllProducts} className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white font-semibold px-8 py-3 rounded-lg hover:opacity-90 transition-all duration-300 transform active:scale-95">
                Browse Products
              </button>
            </div>
          )}

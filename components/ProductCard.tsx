            ...
            <div className="relative w-full overflow-hidden aspect-[4/3] bg-gray-100">
                ...
                {displayMode === 'showcase' && (
                    <div className="absolute inset-x-0 bottom-0 p-4 bg-gradient-to-t from-slate-50/80 via-indigo-50/30/40 to-transparent translate-y-full group-hover:translate-y-0 transition-transform duration-300 ease-in-out flex items-end justify-center pb-6">
                         <button onClick={handleQuickViewClick} className="w-full max-w-[13rem] bg-white/30 backdrop-blur-md border border-white/60 text-slate-900 font-bold px-5 py-3 rounded-xl hover:bg-white/45 transition-colors text-sm shadow-[0_8px_30px_rgb(0,0,0,0.04)] active:scale-95">
                             Quick View
                         </button>
                    </div>
                )}
                ...
                <div className="absolute top-3 left-3 flex flex-col gap-2">
                    {product.isFree && (
                        <span className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-md shadow-[0_8px_30px_rgb(0,0,0,0.04)] animate-fade-in">
                            Free
                        </span>
                    )}
                    {isCouponAvailable && product.couponCode && !product.isFree && settings.features.showSaleBadges && (
                        <button 
                            ...
                            className="bg-purple-600 text-white px-3 py-1 rounded-md text-xs font-bold tracking-wider shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:scale-105 transition-transform font-mono"
                    ...
                    {!product.isFree && product.salePrice && settings.features.showSaleBadges && (
                        <span className="bg-red-500 text-white px-3 py-1 text-xs font-bold uppercase tracking-wider rounded-md shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                            Sale
                        </span>
                    )}
                </div>
                ...
                {settings.features.showFavourites && (
                    <button 
                        ...
                        className="absolute top-3 right-3 bg-white/90 backdrop-blur rounded-full p-2 text-slate-600 hover:text-red-500 hover:bg-white shadow-sm transition-all duration-200 hover:scale-110 z-10"
            ...
            <div className="p-5 flex flex-col flex-grow">
                ...
                {displayMode === 'showcase' ? (
                     <div className="flex items-end justify-between mt-auto pt-4 border-t border-gray-50">
                        ...
                        <button onClick={() => onViewDetails()} className="rounded-full border border-indigo-200/70 bg-white/85 px-4 py-2 text-sm font-black text-primary shadow-sm transition hover:-translate-y-0.5 hover:bg-indigo-50 active:scale-95 flex items-center">

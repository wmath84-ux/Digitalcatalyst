...
const PlatformExperience: React.FC<PlatformExperienceProps> = ({ settings }) => {
  ...
  return (
    <section className="relative overflow-hidden bg-slate-100 bg-gradient-to-br from-slate-100 via-slate-200/80 to-slate-300/70 py-20 text-slate-900 sm:py-24">
      <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(37,99,235,0.35),transparent_30%),radial-gradient(circle_at_82%_8%,rgba(168,85,247,0.24),transparent_28%),linear-gradient(180deg,rgba(255,255,255,0.25),rgba(255,255,255,0.72))]" />
      ...
      <div className="relative container mx-auto px-6">
        ...
        <div className="mt-12 grid gap-4 sm:grid-cols-2 xl:grid-cols-3">
          {capabilities.map((item) => (
            <article key={item.title} className="group relative overflow-hidden rounded-[2rem] border border-white/50 bg-white/70 p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:border-cyan-200/35 hover:bg-white/80 hover:shadow-sm">
              ...
              <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-2xl border border-white/50 bg-white/70 text-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)]">{item.icon}</div>

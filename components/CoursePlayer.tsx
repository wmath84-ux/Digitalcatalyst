...
export default function CoursePlayer({ settings, product, onBack }: { settings: WebsiteSettings; product: ProductWithRating; onBack: () => void; }) {
  ...
  const addNote = () => {
    ...
    setNotes(prev => [{ id: `n-${Date.now()}`, timestamp: videoRef.current ? Math.floor(videoRef.current.currentTime) : null, text: '', createdAt: new Date().toISOString() }, ...prev]);
  };

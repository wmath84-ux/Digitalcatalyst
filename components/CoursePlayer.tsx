    ...
    <div className="relative flex h-full min-h-0 w-full max-w-full flex-col overflow-hidden bg-white text-slate-900">
      ...
      <div className="open-docs-toolbar order-2 flex shrink-0 items-center gap-1.5 overflow-x-auto overscroll-x-contain border-t border-[#D9E7F8] bg-white p-2 shadow-sm sm:gap-2 sm:p-3 lg:order-1 lg:border-b lg:border-t-0 custom-scrollbar">
        ...
        <div className="relative ml-auto shrink-0">
          ...
          {isActionMenuOpen && (
            <div className="absolute right-0 top-12 z-30 w-64 rounded-3xl border border-[#D9E7F8] bg-white p-3 text-left shadow-[0_22px_55px_rgba(8,26,69,0.18)]">
              <button type="button" onClick={() => setHasRuledLines(value => !value)} className="w-full rounded-2xl px-3 py-2 text-left text-sm font-black text-[#081A45] hover:bg-[#F1EEFF]">{hasRuledLines ? 'Hide' : 'Toggle'} Ruled Lines</button>
              <label className="mt-2 flex items-center justify-between rounded-2xl px-3 py-2 text-sm font-black text-[#081A45] hover:bg-[#F8FBFF]">Page Color<input type="color" value={paperColor} onChange={event => setPaperColor(event.target.value)} className="h-8 w-10 cursor-pointer rounded-lg" /></label>
              <label className="mt-2 flex items-center justify-between rounded-2xl px-3 py-2 text-sm font-black text-[#081A45] hover:bg-[#F8FBFF]">Highlight Text<input type="color" value={highlightColor} onChange={event => runHighlightCommand(event.target.value)} className="h-8 w-10 cursor-pointer rounded-lg" /></label>
            </div>
          )}
        </div>

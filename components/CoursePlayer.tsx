...
const QuizPlayer: React.FC<{ file: ProductFile; economySettings: EconomySettings; onQuizReward?: (quizId: string, quizTitle: string, correctAnswers: number, coins: number) => boolean; }> = ({ file, economySettings, onQuizReward }) => {
  ...
  return (
    <div className={`flex h-full min-h-0 overflow-hidden text-slate-900 ${compactQuiz ? 'p-1.5' : 'p-3 sm:p-5 md:p-6'}`}>
      <div className={`mx-auto grid h-full min-h-0 w-full rounded-[1.25rem] border border-white/50 bg-white/60 shadow-[0_8px_30px_rgb(0,0,0,0.04)] backdrop-blur-3xl sm:rounded-[2rem] ${compactQuiz ? 'grid-rows-[auto_auto_minmax(0,1fr)_auto_auto] gap-1.5 p-2' : desktopQuizLayout ? 'max-w-none grid-cols-[minmax(230px,0.7fr)_minmax(0,1.7fr)] grid-rows-[auto_auto_minmax(0,1fr)_auto_auto] gap-4 p-4 md:p-5 xl:p-6' : 'max-w-5xl grid-rows-[auto_auto_auto_minmax(0,1fr)_auto_auto] gap-4 p-4 sm:p-6 md:p-8'}`}>
        <div className={`${compactQuiz ? 'flex-row items-center gap-2' : desktopQuizLayout ? 'col-start-1 row-start-1 flex-col gap-4 rounded-[1.5rem] border border-white/50 bg-white/70 p-4 shadow-sm xl:p-5' : 'gap-4 sm:flex-row sm:items-center sm:justify-between'} flex shrink-0 justify-between`}>
        ...
        {questions.length > 1 && (
          <div className={`${compactQuiz ? 'flex-nowrap gap-1.5 overflow-x-auto p-1.5' : desktopQuizLayout ? 'col-start-1 row-start-3 content-start gap-2 overflow-y-auto p-3' : 'max-h-28 flex-wrap gap-2 overflow-y-auto p-3'} flex shrink-0 rounded-2xl border border-white/50 bg-white/70 custom-scrollbar`}>
        ...
        <div className={`${compactQuiz ? 'rounded-2xl p-3' : desktopQuizLayout ? 'col-start-2 row-span-5 row-start-1 rounded-[1.75rem] p-5 xl:p-7 2xl:p-9' : 'rounded-[1.5rem] p-3 sm:rounded-3xl sm:p-5 md:p-7'} min-h-0 overflow-y-auto border border-white/50 bg-white/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] custom-scrollbar`}>
        ...
        <div className={`${compactQuiz ? 'grid grid-cols-3 gap-1.5' : desktopQuizLayout ? 'col-start-1 row-start-5 grid gap-3 self-end' : 'flex gap-3 sm:items-center sm:justify-between sm:gap-4'} shrink-0`}>

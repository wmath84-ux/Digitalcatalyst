...
export default function CoursePlayer({ settings, product, onBack }: { settings: WebsiteSettings; product: ProductWithRating; onBack: () => void; }) {
  ...
  return <div className="h-screen w-screen bg-gray-900 flex flex-col lg:flex-row">
    ...
    <div className="flex-1 grid min-w-0 lg:grid-cols-[1fr_420px]"><main className="bg-black relative overflow-hidden">{renderMedia()}</main><aside className="hidden lg:block border-l border-white/10 bg-slate-950/85"><div className="p-2 border-b border-slate-700 flex"><button onClick={() => setActiveActionTab('mentor')} className={`flex-1 py-2 ${activeActionTab==='mentor'?'bg-primary text-white':'text-gray-300'}`}>AI</button><button onClick={() => setActiveActionTab('notes')} className={`flex-1 py-2 ${activeActionTab==='notes'?'bg-primary text-white':'text-gray-300'}`}>Notes</button></div>{activeActionTab==='mentor'?<div className="p-3"><div className="mb-3 rounded-2xl border border-white/10 bg-white/5 p-3 text-slate-200"><p className="text-xs uppercase tracking-widest text-indigo-300">AI Mentor</p><p className="mt-1 text-sm">Ask anything about this lesson. Your mobile chat is persisted automatically.</p></div><AiMentor productTitle={product.title} activeContentName={activeFile?.name||null}/></div>:<div className="p-3 space-y-2"> <button onClick={addNote} className="w-full bg-emerald-600 text-white rounded py-2">+ Add Note</button>{notes.map(n=><textarea key={n.id} value={n.text} onChange={e=>updateNote(n.id,e.target.value)} className="w-full rounded p-2" rows={3}/>)}</div>}</aside></div>

...
export default function CoursePlayer({ settings, product, onBack }: { settings: WebsiteSettings; product: ProductWithRating; onBack: () => void; }) {
  ...
  const renderMedia = () => {
    ...
    if (activeFile.type === 'doc') return <div className="h-full w-full bg-white p-4 md:p-8 overflow-y-auto"><div className="mx-auto max-w-5xl rounded-2xl border bg-white shadow p-4"><div className="mb-3 flex flex-wrap items-center gap-2"><span className="mr-2 text-xs uppercase tracking-wider text-slate-500">Google Docs style editor</span><button onClick={() => applyDocFormat('bold')} className="rounded border px-2 py-1 text-xs font-bold">B</button><button onClick={() => applyDocFormat('italic')} className="rounded border px-2 py-1 text-xs italic">I</button><button onClick={() => applyDocFormat('underline')} className="rounded border px-2 py-1 text-xs underline">U</button><button onClick={() => applyDocFormat('insertUnorderedList')} className="rounded border px-2 py-1 text-xs">• List</button><button onClick={() => applyDocFormat('justifyLeft')} className="rounded border px-2 py-1 text-xs">Left</button><button onClick={() => applyDocFormat('justifyCenter')} className="rounded border px-2 py-1 text-xs">Center</button></div><div contentEditable suppressContentEditableWarning onInput={e=>activeFile && localStorage.setItem(`doc-edit-${activeFile.id}`, (e.target as HTMLDivElement).innerHTML)} className="min-h-[65vh] rounded-xl border p-5 outline-none text-[18px] leading-8" dangerouslySetInnerHTML={{__html: localStorage.getItem(`doc-edit-${activeFile.id}`) || activeFile.content || '<h1>Rich Notes</h1><p>Start writing...</p>'}} /></div></div>;

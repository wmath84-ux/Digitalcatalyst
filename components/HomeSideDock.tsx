...
const HomeSideDock: React.FC<HomeSideDockProps> = ({ currentUser, cartCount, wishlistCount, onOpenBlogModal, onOpenFreeModal, onOpenAnnouncementsModal, onNavigateToAllProducts, onNavigateToWishlist, onNavigateToPurchases, onCartClick, onProfileClick }) => {
  ...
  return (
    <aside className={`hidden xl:flex fixed left-5 top-28 z-40 flex-col transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'}`}>
      <div className="rounded-[1.75rem] border border-white/30 bg-white/20 p-3 shadow-2xl backdrop-blur-2xl ring-1 ring-slate-900/5">
        <button onClick={() => setCollapsed(!collapsed)} className="mb-3 flex w-full items-center justify-between rounded-2xl bg-slate-950/90 px-3 py-2 text-sm font-black text-white shadow-lg">
          <span>{collapsed ? '☰' : 'Learning Dock'}</span>
          {!collapsed && <span className="text-white/70">Hide</span>}
        </button>
        {!collapsed && currentUser && (
          <div className="mb-3 rounded-2xl bg-white/70 p-3 text-slate-900 shadow-sm">
            <p className="text-xs font-bold uppercase tracking-widest text-slate-500">EduCoins</p>
            <p className="text-2xl font-black">🪙 {currentUser.eduCoins ?? 120}</p>
          </div>
        )}
        <div className="space-y-2">
          {dockItems.map(([label, icon]) => (
            <button key={label} onClick={actionMap[label]} className="group relative flex w-full items-center gap-3 rounded-2xl bg-white/65 px-3 py-3 font-bold text-slate-700 shadow-sm transition-all hover:-translate-y-0.5 hover:bg-white hover:text-primary hover:shadow-xl">
              <span className="text-xl">{icon}</span>
              {!collapsed && <span>{label}</span>}
              {!collapsed && label === 'Cart' && cartCount > 0 && <span className="ml-auto rounded-full bg-primary px-2 py-0.5 text-xs text-white">{cartCount}</span>}

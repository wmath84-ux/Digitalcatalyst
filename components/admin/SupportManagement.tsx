...
const SupportManagement: React.FC<{ tickets: SupportTicket[]; onUpdate: (updatedTickets: SupportTicket[]) => void; }> = ({ tickets, onUpdate }) => {
  ...
  const visibleTickets = tickets.filter(ticket => (filter === 'All' || ticket.status === filter) && (categoryFilter === 'All' || ticket.category === categoryFilter) && `${ticket.subject} ${ticket.customerName} ${ticket.customerEmail} ${ticket.message}`.toLowerCase().includes(query.toLowerCase()));
                ...
                <div className="grid gap-5 xl:grid-cols-[0.85fr_1.15fr]">
                  ...
                  <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
                    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                      ...
                      <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-black text-slate-500">Mailto ready</span>
                    </div>
                    ...
                    <div className="mt-4 flex flex-col gap-3 sm:flex-row sm:justify-end">
                      ...
                      <button onClick={handleSendReply} disabled={!replyText.trim()} className="rounded-xl bg-gradient-to-r from-indigo-600 to-purple-600 px-6 py-3 font-black text-white disabled:cursor-not-allowed disabled:opacity-60">Send via email</button>
                    </div>
                  </section>
                </div>
              ) : (

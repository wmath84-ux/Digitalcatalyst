...
const StatCard: React.FC<{ title: string; value: string | number; change?: string; changeType?: 'increase' | 'decrease'; icon?: React.ReactNode; colorClass?: string }> = ({ title, value, change, changeType, icon, colorClass = "bg-white/80 backdrop-blur-xl" }) => {
    const changeColor = changeType === 'increase' ? 'text-green-600' : 'text-red-500';
    const arrow = changeType === 'increase' ? '↑' : '↓';

    return (
        <div className={`${colorClass} p-6 rounded-xl shadow-sm border border-slate-200/80 flex flex-col justify-between h-full transition-all hover:shadow-sm`}>
            <div className="flex justify-between items-start">
                <h3 className="text-sm font-medium text-slate-600">{title}</h3>
                {icon && <div className="text-slate-600 p-2 bg-slate-100/80 rounded-lg">{icon}</div>}
            </div>
            <div className="mt-4">
...
const Analytics: React.FC<AnalyticsProps> = ({ orders, products, users, reviews }) => {
    ...
    const DateFilterButtons = () => {
        ...
        return (
             <div className="flex bg-white/80 backdrop-blur-xl rounded-lg p-1 border shadow-sm">
                {ranges.map(range => (
                    <button
                        key={range.value}
    ...
    return (
        <div className="space-y-8 animate-fade-in pb-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="bg-white/80 backdrop-blur-xl p-6 rounded-xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-200/80">
                    ...
                    {topSellingProducts.length > 0 ? (
                        <table className="w-full text-sm text-left">
                            ...
                            <tbody>
                                {topSellingProducts.map((product) => (
                                    <tr key={product.name} className="border-b border-gray-50 last:border-0 hover:bg-slate-100/80 transition-colors">
                        ...
                        <div className="flex flex-col items-center justify-center h-40 text-slate-600 bg-slate-100/80 rounded-lg border border-dashed">

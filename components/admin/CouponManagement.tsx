  ...
  return (
    <MacWindowModal title={coupon ? 'Edit Coupon' : 'Create Coupon'} subtitle="Clean discount setup" onClose={onClose} maxWidth="max-w-lg">
      <form onSubmit={handleSubmit} className="space-y-4 p-6">
        <input name="code" value={formData.code} onChange={handleChange} placeholder="Code e.g. SAVE20" required className="w-full rounded-2xl border bg-slate-100/80 p-4 font-black uppercase tracking-wider" />
        <div className="grid grid-cols-2 gap-4"><select name="type" value={formData.type} onChange={handleChange} className="rounded-2xl border bg-white/80 backdrop-blur-xl p-4"><option value="percentage">Percentage (%)</option><option value="fixed">Fixed (₹)</option></select><input name="value" type="number" value={formData.value} onChange={handleChange} placeholder="Value" required className="rounded-2xl border bg-slate-100/80 p-4" /></div>
        <input name="expiryDate" type="date" value={formData.expiryDate} onChange={handleChange} required className="w-full rounded-2xl border bg-slate-100/80 p-4" />
        <input name="usageLimit" type="number" value={formData.usageLimit} onChange={handleChange} placeholder="Usage limit" required className="w-full rounded-2xl border bg-slate-100/80 p-4" />

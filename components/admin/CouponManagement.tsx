  ...
  return (
    <MacWindowModal title={coupon ? 'Edit Coupon' : 'Create Coupon'} subtitle="Clean discount setup" onClose={onClose} maxWidth="max-w-lg">
      <form onSubmit={handleSubmit} className="space-y-4 p-6">
        ...
        <div className="grid grid-cols-2 gap-4"><select name="type" value={formData.type} onChange={handleChange} className="rounded-2xl border bg-white/10 backdrop-blur-xl p-4"><option value="percentage">Percentage (%)</option><option value="fixed">Fixed (₹)</option></select><input name="value" type="number" value={formData.value} onChange={handleChange} placeholder="Value" required className="rounded-2xl border bg-slate-50 p-4" /></div>

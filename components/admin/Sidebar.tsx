...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onNavigate, onLogout, onSwitchToHome, isOpen, onClose }) => {
    ...
    const containerClasses = isOpen 
        ? "fixed inset-y-0 left-0 z-50 w-72 bg-gradient-to-b from-slate-100 via-slate-200/80 to-slate-300/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] transition-transform transform translate-x-0"
        : "hidden md:flex md:w-72 bg-gradient-to-b from-slate-100 via-slate-200/80 to-slate-300/70 shadow-[0_8px_30px_rgb(0,0,0,0.04)] flex-col md:fixed md:left-0 md:top-0 md:bottom-0 md:z-40 md:translate-x-[-248px] md:hover:translate-x-0 md:transition-transform md:duration-300";

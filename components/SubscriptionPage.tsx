...
const SubscriptionPage: React.FC<{
  ...
  const accentLines = ['from-[#1A73E8] to-[#C2E7FF]', 'from-[#34A853] to-[#C2E7FF]', 'from-[#FBBC04] to-[#C2E7FF]'];
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#F8FAFD] pb-32 text-[#202124]">
      <div className="pointer-events-none absolute inset-0 bg-[linear-gradient(135deg,#E8F0FE_0%,#D3E3FD_52%,#C2E7FF_100%)]" />
      <div className="pointer-events-none absolute left-1/2 top-24 h-72 w-[46rem] -translate-x-1/2 rounded-full bg-[#1A73E8]/10 blur-3xl" />

      <main className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-5 py-8 sm:px-8 lg:px-10">
        <button onClick={onBack} className="w-fit rounded-full border border-[#D2E3FC] bg-white/95 px-5 py-2.5 text-sm font-semibold text-[#174EA6] shadow-[0_18px_60px_rgba(26,115,232,0.22)] backdrop-blur-2xl transition hover:-translate-y-0.5 hover:bg-[#E8F0FE]">
          ← Back
        </button>

        <section className="flex flex-1 flex-col items-center justify-center py-10">
          <div className="text-center">
            <p className="text-sm font-medium uppercase tracking-[0.32em] text-[#5F6368]">Premium learning access</p>
            ...
            {plans.map((plan, index) => {
              ...
              return (
                <article key={plan.id} className={`group relative flex min-h-[25rem] flex-col rounded-[2.2rem] border border-[#D2E3FC] bg-white/95 px-5 pb-6 pt-5 text-center shadow-[0_30px_90px_rgba(26,115,232,0.16)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 ${isHighlighted ? 'scale-[1.03]' : ''}`}>
                  {isHighlighted && <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#D3E3FD] px-4 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#174EA6] shadow-lg shadow-[#D2E3FC]">{plan.badge || 'Best Value'}</span>}
                  ...
                  {(activeDiscount || validAppliedCoupon) && (
                    <div className="mx-auto mt-3 w-full rounded-2xl border border-[#D2E3FC] bg-[#F8FAFD] p-3 text-xs font-semibold text-[#137333]">
                  ...
                  <div className="mt-4 rounded-3xl border border-[#E0E3EB] bg-white/70 p-3 text-left shadow-inner">
                    <p className="text-[10px] font-black uppercase tracking-[0.18em] text-[#5F6368]">Have a coupon?</p>
                    <div className="mt-2 flex gap-2">
                      ...
                      <button
                        ...
                        className={`rounded-2xl px-3 py-2 text-xs font-black transition active:scale-95 disabled:cursor-not-allowed disabled:bg-[#E0E3EB] disabled:text-[#5F6368] ${validAppliedCoupon ? 'bg-[#FCE8E6] text-[#C5221F] hover:bg-[#FAD2CF]' : 'bg-[#1A73E8] text-white hover:-translate-y-0.5 hover:bg-[#1967D2]'}`}
                      >
                  ...
                  <div className="mt-5 space-y-2">
                    <button disabled={allUnlocked} onClick={handlePlanCheckout} className={`mx-auto block rounded-full px-7 py-2.5 text-[11px] font-bold text-white shadow-[0_10px_20px_rgba(26,115,232,0.24)] transition active:scale-95 ${allUnlocked ? 'cursor-not-allowed bg-[#E0E3EB] text-[#5F6368]' : 'bg-[#1A73E8] hover:-translate-y-0.5 hover:bg-[#1967D2]'}`}>
                      {allUnlocked ? 'Already Active' : validAppliedCoupon || activeDiscount ? 'Apply & Activate' : 'Get this plan'}
                    </button>
                    
                    {coinPrice > 0 && (
                      <button disabled={allUnlocked || !canPayWithCoins} onClick={() => onActivatePlanWithCoins?.(plan)} className="mx-auto block rounded-full border border-[#D2E3FC] bg-white/95 px-5 py-2 text-[11px] font-bold text-[#B06000] shadow-sm transition hover:-translate-y-0.5 active:scale-95 disabled:cursor-not-allowed disabled:opacity-50">

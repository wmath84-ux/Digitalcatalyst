...
const SubscriptionPage: React.FC<{economySettings: EconomySettings; activeCoinDiscount?: ActiveCoinDiscount | null; onConsumeCoinDiscount?: () => void; settings: WebsiteSettings; products: ProductWithRating[]; purchasedProductIds: number[]; onBack: () => void; onActivatePlan: (plan: Plan) => void; currentUser?: User | null; onActivatePlanWithCoins?: (plan: Plan) => void;}> = ({ economySettings, activeCoinDiscount = null, onConsumeCoinDiscount, settings, products, purchasedProductIds, onBack, onActivatePlan, currentUser, onActivatePlanWithCoins }) => {
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-[#eaf3fb] pb-20 text-slate-900 sm:pb-32">
      ...
      <div className="pointer-events-none absolute left-1/2 top-20 h-52 w-[22rem] -translate-x-1/2 rounded-full bg-cyan-100/50 blur-3xl sm:top-24 sm:h-72 sm:w-[46rem]" />

      <main className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-3 py-4 sm:px-8 sm:py-8 lg:px-10">
        <button onClick={onBack} className="w-fit rounded-full border border-white/80 bg-white/55 px-4 py-2 text-xs font-semibold text-slate-700 shadow-[0_14px_35px_rgba(116,153,176,0.18)] backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white/80 sm:px-5 sm:py-2.5 sm:text-sm">← Back</button>

        <section className="flex flex-1 flex-col items-center justify-start py-6 sm:justify-center sm:py-10">
          <div className="text-center">
            <p className="text-[11px] font-medium uppercase tracking-[0.2em] text-slate-500 sm:text-sm sm:tracking-[0.32em]">Premium learning access</p>
            <h1 className="mt-2 text-2xl font-light tracking-wide text-slate-900 sm:mt-3 sm:text-4xl">Pricing table example</h1>
            <p className="mx-auto mt-3 max-w-xl px-1 text-xs leading-5 text-slate-500 sm:mt-4 sm:px-0 sm:text-sm sm:leading-6">Choose your trusted subscription plan and unlock premium notes, courses, and resources in one clean learning space.</p>
          </div>

          <div className="mt-6 grid w-full max-w-4xl gap-5 sm:mt-10 sm:gap-8 md:grid-cols-3 md:gap-10">
            {plans.map((plan, index) => {
              ...
              return (
                <article key={plan.id} className={`group relative flex min-h-0 flex-col rounded-[1.75rem] border border-white/80 bg-[#edf7fb]/80 px-4 pb-5 pt-4 text-center shadow-[18px_22px_42px_rgba(118,157,180,0.22),-12px_-14px_28px_rgba(255,255,255,0.88)] backdrop-blur-xl transition duration-300 hover:-translate-y-1 sm:min-h-[25rem] sm:rounded-[2.2rem] sm:px-5 sm:pb-6 sm:pt-5 ${isHighlighted ? 'sm:scale-[1.03]' : ''}`}>
                  {isHighlighted && <span className="absolute -top-3 left-1/2 -translate-x-1/2 whitespace-nowrap rounded-full bg-violet-400 px-3 py-1 text-[9px] font-bold uppercase tracking-[0.14em] text-white shadow-[0_10px_24px_rgba(168,85,247,0.22)] sm:px-4 sm:text-[10px] sm:tracking-[0.18em]">{plan.badge || 'Best Value'}</span>}
                  <h2 className="text-sm font-medium text-slate-700 sm:text-base">{plan.name}</h2>
                  ...
                  <div className="mt-3 flex items-start justify-center text-slate-950 sm:mt-4">
                    <span className="mt-1.5 text-xl font-light sm:mt-2 sm:text-2xl">₹</span>
                    <span className="text-4xl font-light leading-none tracking-tight sm:text-5xl">{finalPlanPrice}</span>
                  </div>
                  <p className="mt-1.5 text-[11px] font-medium text-slate-500 sm:mt-2 sm:text-xs">Price Example</p>
                  {activeDiscount && <div className="mx-auto mt-3 w-full rounded-2xl border border-emerald-100 bg-white/55 p-2.5 text-[11px] font-semibold text-emerald-700 sm:p-3 sm:text-xs"><div className="flex justify-between"><span>Subtotal</span><span>₹{plan.price}</span></div><div className="flex justify-between"><span>EduCoin Discount</span><span>-₹{activeDiscount.amount}</span></div><div className="mt-1 flex justify-between font-black"><span>Final Price</span><span>₹{finalPlanPrice}</span></div></div>}
                  <p className="mx-auto mt-3 min-h-0 max-w-[16rem] text-[11px] leading-5 text-slate-400 sm:mt-4 sm:min-h-10 sm:max-w-[12rem]">{plan.description}</p>

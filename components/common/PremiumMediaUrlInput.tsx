...
import { MEDIA_LINK_NOT_LOADING_MESSAGE, MEDIA_PASTE_URL_MESSAGE } from '../../utils/mediaMode';
...
const PremiumMediaUrlInput: React.FC<{
  ...
  const message = status === 'invalid' ? 'Please paste a valid https media URL.' : status === 'warning' ? MEDIA_LINK_NOT_LOADING_MESSAGE : status === 'valid' ? `${kind === 'audio' ? 'Audio' : kind === 'video' ? 'Video' : 'Media'} URL ready.` : MEDIA_PASTE_URL_MESSAGE;
  ...
  return <section className="overflow-hidden rounded-[1.75rem] border border-[#D9E7F8] bg-white/85 shadow-[0_20px_60px_rgba(23,105,255,0.12)] backdrop-blur-xl">
    <div className="bg-gradient-to-r from-[#1769FF] to-[#7B61FF] p-4 text-white">
      <p className="text-xs font-black uppercase tracking-[0.24em] text-white/75">URL-first media</p>
      <h3 className="mt-1 text-xl font-black">{label || `Paste ${kind} URL`}</h3>
      <p className="mt-2 text-sm font-bold text-white/85">{helperText || 'Use a public https URL. File upload is a future Storage feature.'}</p>
    </div>
    <div className="grid gap-4 p-4 lg:grid-cols-[1fr_280px]">
      <div>
        ...
        {helperOpen ? <div className="mt-3 rounded-2xl border border-[#D9E7F8] bg-[#F8FBFF] p-4 text-sm font-bold text-[#536178]">Upload your file to a public hosting provider or Google Drive, set sharing to public/anyone with link, copy the direct/share URL, paste it here, then preview and save.</div> : null}
      </div>

...
const AuthPage: React.FC<AuthPageProps> = ({ settings, onOtpAuthenticate, onBack }) => {
    ...
    return (
        <div className="min-h-screen bg-[#070b1f] text-white relative overflow-hidden flex items-center justify-center p-4 sm:p-6">
            ...
            <button onClick={onBack} className="absolute top-5 left-5 z-10 text-white/80 hover:text-white font-semibold">&larr; Back</button>

            <div className="relative z-10 w-full max-w-6xl grid lg:grid-cols-[1.05fr_0.95fr] rounded-[2rem] overflow-hidden bg-white/10 backdrop-blur-2xl border border-white/15 shadow-2xl">
                <section className="p-8 sm:p-12 lg:p-14 bg-gradient-to-br from-white/15 to-white/5">
                    <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/15 text-sm text-blue-100 mb-8">
                    ...
                    <p className="mt-5 text-lg text-slate-200 max-w-xl">Login or sign up with mobile OTP, save favourites, unlock purchases, watch private video embeds, and continue reading inside a Google Docs inspired workspace.</p>
                ...
                <section className="bg-white/10 backdrop-blur-xl text-slate-900 p-6 sm:p-10 lg:p-12">
                    <div className="mb-8">
                        ...
                        <p className="text-slate-500 mt-2">We will use your name for reviews, email for receipts, and mobile for OTP access.</p>
                    </div>
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <label className="block">
                            ...
                            <div className="mt-2 flex rounded-2xl border border-slate-200 bg-slate-50 overflow-hidden focus-within:ring-4 focus-within:ring-blue-100 focus-within:border-primary">
                                <span className="px-4 py-3 bg-slate-100 text-slate-500 font-bold">+91</span>
                        ...
                        <div className="grid sm:grid-cols-[1fr_auto] gap-3">
                            ...
                            <button type="button" onClick={handleSendOtp} className="px-5 py-3 rounded-2xl bg-slate-900 text-white font-bold hover:bg-primary transition-colors">Send OTP</button>
                        </div>
                        ...
                        <button type="submit" className="w-full bg-gradient-to-r from-primary to-accent text-white font-black px-8 py-4 rounded-2xl hover:shadow-xl hover:-translate-y-0.5 transition-all">Continue to learning store</button>
                    </form>
                    <p className="mt-5 text-xs text-slate-400 text-center">Prototype note: OTP is simulated locally; connect Firebase/Auth provider for production SMS delivery.</p>
                </section>
            </div>
        </div>
    );

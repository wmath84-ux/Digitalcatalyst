...
const Icon: React.FC<{ name: string }> = ({ name }) => {
    ...
    return (
        <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-[var(--primary)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
...
const statusColors: { [key in UpcomingFeatureItem['status']]: string } = {
    'In Development': 'border-[#D9E7F8] bg-[#E8F2FF] text-[#1769FF]',
    'Coming Soon': 'border-[#D9E7F8] bg-[#E8F2FF] text-[#1769FF]',
    'Beta': 'border-[#BFE8D4] bg-[#EAF8EF] text-[#22A06B]',
};
...
const UpcomingFeatures: React.FC<UpcomingFeaturesProps> = ({ settings, title, features, onOpenCommunity }) => {
  ...
  return (
    <section 
      id="upcoming"
      ref={sectionRef}
      className={`tagmaster-section-theme relative overflow-hidden bg-sky-50 bg-gradient-to-br from-sky-50 via-indigo-100/50 to-violet-100 py-24 text-[var(--text-body)] ${settings.animations.enabled ? 'stagger-animate-container' : ''}`}
    >
      ...
      <div className="container relative z-10 mx-auto px-6">
        <div className="animate-child animate-delay-1 mx-auto mb-16 max-w-3xl rounded-[28px] border border-[var(--border-soft)] bg-white p-8 text-center backdrop-blur-xl">
          <p className="text-sm font-bold uppercase tracking-[0.35em] text-[var(--primary)]">Roadmap</p>
          <h2 className="mt-3 text-4xl font-extrabold text-[var(--text-heading)]">{title}</h2>
        ...
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-3">
          {features.map((feature, index) => (
            <div key={feature.id} className={`animate-child animate-delay-${index + 2} product-card-shine group flex h-full flex-col rounded-[28px] border border-[var(--border-soft)] bg-white p-8 text-center shadow-[var(--shadow-card)] shadow-black/5 backdrop-blur-xl transition-all duration-300 hover:-translate-y-1 hover:border-[var(--border-active)] hover:shadow-[var(--shadow-card)]`}>
                <div className="mb-6 flex justify-center">
                    <div className="flex h-20 w-20 items-center justify-center rounded-[28px] border border-[var(--border-soft)] bg-white shadow-[var(--shadow-card)] transition-transform duration-300 group-hover:scale-110">
                        <Icon name={feature.icon} />
                    </div>
                </div>
                <h3 className="text-xl font-bold text-[var(--text-heading)] mb-2">{feature.title}</h3>
                <p className="mt-2 flex-grow text-sm leading-relaxed text-[var(--text-body)]">{feature.description}</p>
                <div className="mt-6 border-t border-white/50 pt-6">
                    ...
                    {feature.title === 'Community Forum' && onOpenCommunity && (
                        <button type="button" onClick={onOpenCommunity} className="mt-4 w-full rounded-full border border-white/70 bg-gradient-to-r from-[#1769FF] to-[#7B61FF] px-5 py-3 text-sm font-black text-white shadow-[var(--shadow-blue)] transition hover:-translate-y-0.5">

...
const MacWindowModal: React.FC<MacWindowModalProps> = ({ title, subtitle, children, onClose, maxWidth = 'max-w-3xl', zIndex = 'z-[80]', className = '' }) => {
  ...
  return (
    <div className={`fixed inset-0 ${zIndex} flex items-center justify-center p-4 bg-slate-900/20 backdrop-blur-xl animate-fade-in`} role="dialog" aria-modal="true" aria-labelledby="mac-modal-title" onMouseDown={onClose}>

...
const WelcomeOverlay: React.FC<WelcomeOverlayProps> = ({ onAnimationComplete }) => {
  ...
  return (
    <div className={`fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/75 px-4 backdrop-blur-2xl ${isLeaving ? 'welcome-overlay-exit' : 'welcome-overlay-enter'}`}>
      <div className="welcome-circle-shell relative flex h-[min(82vw,24rem)] w-[min(82vw,24rem)] items-center justify-center rounded-full bg-gradient-to-br from-white via-cyan-100 to-sky-200 p-7 text-center text-slate-900 shadow-[0_24px_90px_rgba(14,165,233,0.38),0_0_70px_rgba(103,232,249,0.28)] sm:p-10">
        ...
        <div className="welcome-spark absolute left-[18%] top-[17%] h-3 w-3 rounded-full bg-cyan-300 shadow-[0_0_18px_rgba(34,211,238,0.85)]" />
        <div className="relative z-10 flex max-w-[18rem] flex-col items-center">
          ...
          <p className="welcome-subtitle mt-4 text-xs font-bold uppercase tracking-[0.42em] text-cyan-700 opacity-0 sm:text-sm">Digital Catalyst</p>
        </div>
      </div>
    </div>
  );

    ...
    >
      ...
      <div className="relative container mx-auto px-4 sm:px-6">
        <div className="mx-auto max-w-3xl text-center">
          <p className="text-xs font-black uppercase tracking-[0.26em] text-[var(--primary)] sm:text-sm sm:tracking-[0.32em]">Trusted learning platform</p>
          <h2 className="mt-3 text-3xl font-black leading-tight sm:mt-4 sm:text-5xl">Why Choose Digital Catalyst?</h2>
          <p className="mt-3 text-base leading-7 text-[var(--text-body)] sm:mt-5 sm:text-lg sm:leading-8">
        ...
        <div className="mt-8 grid grid-cols-1 gap-4 sm:mt-12 sm:gap-5 md:grid-cols-3">
          {trustItems.map((item) => (
            <article key={item.title} className="group relative overflow-hidden rounded-[1.5rem] border border-[var(--border-soft)] bg-white/80 p-5 text-center shadow-[var(--shadow-card)] backdrop-blur-2xl transition duration-300 hover:-translate-y-1 hover:border-[var(--border-active)] hover:bg-white/80 hover:shadow-sm sm:rounded-[2rem] sm:p-7">
              ...
              <div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-[28px] border border-[var(--border-soft)] bg-white text-2xl shadow-[var(--shadow-card)] sm:mb-5 sm:h-16 sm:w-16 sm:text-3xl">{item.icon}</div>
              <h3 className="text-lg font-black text-[var(--text-heading)] sm:text-xl">{item.title}</h3>
              <p className="mt-2 text-sm leading-6 text-[var(--text-body)] sm:mt-3 sm:leading-7">{item.text}</p>
            </article>
          ))}
        </div>
      </div>

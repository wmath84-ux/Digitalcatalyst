...
type ProductCardProps = {
  ...
  onView: (product: ProductWithRating) => void;
};
...
export default function ProductCard({
  ...
  const isSale = tags.some(t => t.toLowerCase().includes('sale')) || !!product.salePrice;
  const isBoard = tags.some(t => t.toLowerCase().includes('board')) || product.category?.toLowerCase().includes('board');
    ...
    <div className="flex flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        ...
        <p className="text-sm text-slate-500 line-clamp-1">
          by {product.category || 'Eduvora Instructor'}
        </p>

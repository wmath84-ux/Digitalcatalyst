...
const thumb = (product: ProductWithRating) => product.images?.[0];

const ProductCover: React.FC<{ product: ProductWithRating; compact?: boolean }> = ({ product, compact }) => {
...
const MobileAppHome: React.FC<MobileAppHomeProps> = ({
  ...
  const allPreview = useMemo(() => visibleProducts.filter(product => !purchasedProductIds.includes(product.id)).filter(product => {
  ...
  const chips = [
    ...
    { label: 'Top Rated', icon: '⭐', action: onNavigateToAllProducts },
    { label: 'Coupons', icon: '🏷️', action: onNavigateToAllProducts, count: coupons.filter(c => c.isActive).length },

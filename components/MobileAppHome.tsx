...
const MobileAppHome: React.FC<MobileAppHomeProps> = ({
  ...
  return (
    <div className="min-h-[100dvh] bg-[radial-gradient(circle_at_12%_3%,rgba(191,215,255,0.78),transparent_30%),radial-gradient(circle_at_92%_12%,rgba(220,203,255,0.52),transparent_28%),linear-gradient(180deg,#F5F9FF_0%,#EEF6FF_44%,#FFFFFF_100%)] px-4 pb-44 pt-[max(14px,env(safe-area-inset-top))] text-[#64708F]">
      ...
      <section className="mt-7"><SectionHead title="All Products" subtitle="Browse all premium learning products." onViewAll={onNavigateToAllProducts} /><div className="space-y-3">{allPreview.length > 0 ? allPreview.map(product => <article key={product.id} className="relative rounded-[26px] border border-[#D8E6FF] bg-white p-3 shadow-[0_16px_42px_rgba(11,99,255,0.08)]"><button onClick={() => onToggleWishlist(product.id)} className="absolute right-4 top-4 z-30 grid h-9 w-9 place-items-center rounded-full bg-white/90 shadow">{wishlist.includes(product.id) ? '❤️' : '♡'}</button><div className="flex gap-3"><div className="relative w-24 shrink-0 overflow-hidden rounded-[22px] aspect-[6/7]"><ProductCover product={product} slot="homeList" /><button type="button" onClick={() => onViewProduct(product)} aria-label={`Open ${product.title}`} className="absolute inset-0 z-10 focus-visible:outline focus-visible:outline-4 focus-visible:outline-offset-[-4px] focus-visible:outline-[#0B63FF]" /></div><div className="min-w-0 flex-1 pr-10"><div className="flex max-w-full flex-wrap items-center gap-2 overflow-visible pb-0.5">{purchasedProductIds.includes(product.id) ? (<span className="inline-flex max-w-full shrink-0 items-center rounded-full border border-emerald-300/70 bg-emerald-500/15 px-2.5 py-1 text-[9px] font-black uppercase leading-none tracking-[0.08em] text-emerald-700 shadow-[0_6px_16px_rgba(5,150,105,0.16)] ring-1 ring-emerald-100/80">Purchased</span>) : (<span className="inline-flex max-w-full items-center rounded-full border border-emerald-300/70 bg-emerald-500/15 px-2.5 py-1 text-[10px] font-black leading-none text-emerald-700 shadow-[0_6px_16px_rgba(5,150,105,0.16)] ring-1 ring-emerald-100/80">{product.category || 'Learning'}</span>)}</div><h3 className="mt-2 line-clamp-1 text-base font-black text-[#081A44]"><button type="button" onClick={() => onViewProduct(product)} className="block w-full truncate text-left focus-visible:rounded focus-visible:outline focus-visible:outline-2 focus-visible:outline-[#0B63FF]">{product.title}</button></h3><p className="mt-1 line-clamp-2 text-xs font-semibold leading-5 text-[#64708F]">{product.description}</p><div className="mt-2 flex items-center justify-between"><p className="text-[11px] font-bold text-[#64708F]">⭐ <span className="text-[#FFB020]">{product.rating.toFixed(1)}</span> ({product.reviewCount})</p><span className="font-black text-[#081A44]">{currency(product)}</span></div><button onClick={() => onViewProduct(product)} className="mt-2 rounded-xl bg-[#0B63FF] px-3 py-2 text-xs font-black text-white">View Details</button></div></div></article>) : <div className="rounded-[24px] border border-[#D8E6FF] bg-white p-5 text-center font-bold text-[#64708F]">No products found. Try another search.</div>}</div></section>
    </div>
  );
};
